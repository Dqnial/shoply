...
const ProductSkeleton = () => (
  <div className="flex items-center gap-3 sm:gap-4 p-2 sm:p-3 rounded-[20px] animate-pulse">
    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-muted flex-shrink-0" />
  ...
  return (
    <>
      ...
      <CommandDialog
        ...
        className="w-[calc(100%-2.5rem)] sm:max-w-[650px] rounded-[28px] overflow-hidden shadow-2xl border-none"
      >
        <div className="flex items-center border-b px-4">
          <CommandInput
            ...
            // Скрываем стандартную иконку, чтобы поле было полностью пустым
            className="h-16 text-base font-medium placeholder:text-muted-foreground/50 border-none focus:ring-0 w-full [&_svg]:hidden"
          />
        </div>
              ...
              <CommandGroup
                heading={
                  <span className="px-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                    {searchValue ? "Результаты поиска" : "Популярные товары"}
                  </span>
                }
              >
                <div className="space-y-1 mt-2">
                  {products.map((product) => (
                    <CommandItem
                      ...
                      className="flex items-center gap-3 sm:gap-4 p-2 sm:p-3 rounded-[20px] cursor-pointer data-[selected=true]:bg-primary/5 transition-all group"
                    >
